...
<x-orbitali::tab-container id="{{$id}}" :data-repeater-count="$that->count" :data-repeater-names="json_encode($that->inputNames)">
    @for ($i = 1; $i <= $that->count; $i++) @php($title=trans(["native.panel.index.$i",$i])) 
