...
@section('content')
   {{ html()->modelForm($node, 'PUT',  route('panel.node.update', $node->id))->acceptsFiles()->open() }}
    <div class="block block-rounded block-bordered invisible" data-toggle="appear">
        <div class="block-header block-header-default sticky-top">
            ...
            <div class="block-options">
                ...
                <a href="{{route("panel.structure.edit",[$node->structure->id,"model_id"=>$node->id])}}"
        ...
        <div class="block-content">
            {!! \Orbitali\Foundations\Helpers\Structure::renderStructure($node->structure->data) !!}
        </div>
    </div>
    {{ html()->form()->close() }}
@endsection